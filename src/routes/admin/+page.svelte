<script lang="ts">
	...
	import { loadCounts,counts } from '$lib/stores/count';
			...
			<div class="flex w-full flex-col">
				...
				<div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
					<div class="items-center rounded-xl bg-white p-4 shadow">
						...
						<p class="text-3xl font-bold text-blue-600">{$counts.posts}</p>
					</div>
					<div class="rounded-xl bg-white p-4 shadow">
						<h2 class="text-lg font-semibold text-gray-700">Total Pengguna</h2>
						<p class="text-3xl font-bold text-green-600">{$counts.users}</p>
					</div>
					<div class="rounded-xl bg-white p-4 shadow">
						<h2 class="text-lg font-semibold text-gray-700">Komentar</h2>
						<p class="text-3xl font-bold text-purple-600">{$counts.comments}</p>
					</div>
				</div>
			</div>
		{/if}
